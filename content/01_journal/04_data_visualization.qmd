...
# Plotting cumulative cases over time
```{r}
...
```
# Visualizing the distribution of the mortality rate (deaths / population)
...
# Joining the latitude/longitude data with the COVID-19 data
```{r}
plot_data <- left_join(world, adjusted_data_latest, by = c("region" = "location"))
```

# Visualization of the distribution of the mortality rate or case-fatality rate
```{r}
ggplot() +
